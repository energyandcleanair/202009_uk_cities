table_impact <- function(mc.city, tc.tomtom, tc.apple, tc.mapbox, n_day, date_from, date_to, save=T){
  ...
  t.impact <- mc.city %>%
    ...
    filter(process_id %in% c("anomaly_gbm_lag1_city_mad",
                             "anomaly_percent_gbm_lag1_city_mad")) %>%
    mutate(process_id=recode(process_id,
                             "anomaly_percent_gbm_lag1_city_mad"="relative",
                             "anomaly_gbm_lag1_city_mad"="absolute")) %>%
    filter(date>=lubridate::date(date_from),
           date<=lubridate::date(date_to)) %>%
    group_by(region_id, poll, process_id) %>%
    arrange(date) %>%
    #TODO MIN doesn't work (rolling average is wrong)
