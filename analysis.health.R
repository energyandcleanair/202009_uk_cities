...
date_to <- "2020-05-12"
max_n_day <- lubridate::yday(date_from)

m <- rcrea::measurements(city=city,
                         deweathered = NULL,
                         date_from=lubridate::date(date_from)-lubridate::days(max_n_day),
                         date_to=date_to,
                         source="defra",
                         with_metadata = T,
                         with_geometry = T)
...
health.run <- function(m, n_day, date_from, date_to){
  ...
  hi.detailed <- health.impact(m.scenarios, date_from=date_from, date_to=date_to)
  ...
  plot_anomaly_average(m.lockdowns, date_from=date_from, date_to=date_to, filename=paste0("plot_anomaly_lockdown_avg_",n_day,"day.jpg"))
...
health.run(m, max_n_day, date_from, date_to)
health.run(m, NULL, date_from=date_from, date_to=date_to)
